    ...
    def gen_wrapper(dalipipeline, memory_format):
        for data in dalipipeline:
            if "data" in data[0].keys():
                ...
                yield input, target
            elif "data1" in data[0].keys() and "data2" in data[0].keys():
                input1 = data[0]["data1"].contiguous(memory_format=memory_format)
                input2 = data[0]["data2"].contiguous(memory_format=memory_format)
                target = torch.reshape(data[0]["label"], [-1]).cuda().long()
...
class HybridTrainPipe(Pipeline):
    def __init__(self, two_crop, batch_size, num_threads, device_id, data_dir, crop):
    ...
    def define_graph(self):
        ...
        if self.two_crop:
            images2 = self.res(images)
            output2 = self.cmnp(images2, mirror=self.coin())
            return [output1, output2, self.labels]
...
def get_imagenet_iter_dali(type, data_root, batch_size, num_threads, device_id, two_crop=False, crop=224, val_size=256):
    if type == 'train':
        pip_train = HybridTrainPipe(batch_size=batch_size, num_threads=num_threads, device_id=device_id,
                                    data_dir=os.path.join(data_root, 'train'), crop=crop, two_crop=two_crop)
        pip_train.build()
        print(f'pip_train.epoch_size("Reader"): {pip_train.epoch_size("Reader")}')
        print(f"pip_train.two_crop: {two_crop}")
        if two_crop:
            dali_iter_train = DALIClassificationIterator(
                pip_train,
                label=["data1", "data2", "label"],
...
class ImageNetDali:
    def __init__(self, args):
        ...
        train_pipeline = get_imagenet_iter_dali(type='train', data_root=data_root, batch_size=args.batch_size,
                                                num_threads=args.workers, two_crop=args.two_crop, crop=224, device_id=0)
        self.train_loader = DALIWrapper(train_pipeline, memory_format=memory_format)
        val_pipeline = get_imagenet_iter_dali(type='val', data_root=data_root, batch_size=args.batch_size,
